...
public class ChunkPartsGenerator
{
	...
	public ChunkPartsGenerator(Random rand)
	{
		random = rand;
	...
	public void shapeLand(World world, int chunkX, int chunkZ, byte[] blocks)
	{
		byte b0 = 2;
		int k = b0 + 1;
	...
	public void decorateLand(World world, int chunkX, int chunkZ, byte[] blocks)
	{
		byte b0 = 63;
		double d0 = 0.03125D;
		...
		for (int x = 0; x < 16; ++x)
		{
			for (int z = 0; z < 16; ++z)
			{
				...
				byte surfaceBlock, groundBlock;

				//Biome biome = ((CraftWorld) world).getBiome(chunkX + x, chunkZ + z);
				
				surfaceBlock = (byte) Material.GRASS.getId();
				groundBlock = (byte) Material.DIRT.getId();

				byte b1 = surfaceBlock;
				byte b2 = groundBlock;
